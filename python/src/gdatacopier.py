...
try:
	...
	print "GDataCopier failed to import Google Data libraries, please fetch and install"
	print "these from http://code.google.com/p/gdata-python-client/"
	sys.exit(1)
...
class GDataCopier:
	...
	def cache_document_lists(self):
		self._cached_doc_list	= self.get_document_list()
		self._cached_sheet_list = self.get_spreadsheet_list()
		self._cached_presen_list = self.get_presentation_list()
	...
	# Returns the currently cached list of presentations
	def get_cached_presentation_list(self):
		return self._cached_presen_list
	...
	# Gets a live list of presentations from Google
	def get_presentation_list(self):
		return self._get_item_list('presentation')
	...
	def _get_item_list(self, item_type = None):
		...
		# Create a custom dictionary of things we want
